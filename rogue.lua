local success, err = pcall(function()
	local library = {
		...
		colored = {},
		configuration = {
			hideKeybind = Enum.KeyCode.BackSlash,
					...
					if getsynasset then
						...
						elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
							local x, e = pcall(function()
								...
								local fold = isfolder("./Aisar UI")
								if not fold then
									makefolder("./Aisar UI")
								end
								fold = isfolder("./Aisar UI/Themes")
								if not fold then
									makefolder("./Aisar UI/Themes")
								end
								fold = isfolder("./Aisar UI/Themes/SynapseAssetsCache")
								if not fold then
									makefolder("./Aisar UI Themes/SynapseAssetsCache")
								end
								if not fold or not isfile("./Aisar UI/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
									local res = game:HttpGet(string.sub(image, 15))
									if res ~= nil then
										writefile("./Aisar UI/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
									end
								end
								return getsynasset(readfile("./Aisar UI/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
							end)
							if x and e ~= nil then
								return e
							end
	...
	function library:CreateWindow(options, ...)
		...
		if windowName and #windowName > 0 and library.WorkspaceName == "Aisar UI" then
			library.WorkspaceName = convertfilename(windowName, "Aisar UI")
		end
			...
			function tabFunctions:CreateSection(options, ...)
						...
						if submenuOpen == dropdown or submenuOpen == nil then
							if dropdownEnabled then
								...
								if not options.DisablePrecisionScrolling then
									...
									local upkey = options.ScrollUpButton or library.scrollupbutton or Enum.KeyCode.Up
									local downkey = options.ScrollDownButton or library.scrolldownbutton or Enum.KeyCode.Down
				...
				if isfolder and makefolder and listfiles and readfile and writefile then
					function sectionFunctions:AddPersistence(options, ...)
						...
						local realDropdownHolderList = Instance_new("UIListLayout")
						local dropdownEnabled = false
						if not isfolder("./Aisar UI") then
							makefolder("./Aisar UI")
						end
						local common_string = "./Aisar UI/" .. tostring(custom_workspace or library.WorkspaceName)
						local function resolvelist(nofold)
							if custom_workspace ~= options.Workspace then
								custom_workspace = options.Workspace
								common_string = "./Aisar UI/" .. tostring(custom_workspace or library.WorkspaceName)
							end
		...
		function windowFunctions:CreateDesigner(options, ...)
			...
			options = options or {}
			options.Image = options.Image or 4565680085
			...
			local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
				Name = "Image Asset ID",
				Placeholder = "rbxassetid://4565680085",
